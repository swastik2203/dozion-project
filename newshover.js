...
.then(completeData => {
  ...
  final_news.map((values) => {
    ...
    html += `
      <div class="news-item-indexpage" style="width:80%;">
        <img src=${values.related_image} style="height:70px; width:70px;" >
        <p>${headline}</p>
      </div>
    ` ;
  });
  document.getElementById('news-hover').innerHTML = html;
})
.catch(err => {
	console.error(err);
});
